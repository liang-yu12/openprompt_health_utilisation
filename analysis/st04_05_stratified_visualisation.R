...
# Read in data
predicted_count <- read_csv("output/st04_04_stratum_predicted_value.csv")


# 1. Previous COVID admission: ----
hos_stratum <- read_csv("output/st04_04_stratum_hospitalisation.csv") %>% 
...
ggsave(hos_plots, file = "output/st04_05_stratum_hos_visits.svg",
       device = "svg",
       width=12, height=5, units = "in", dpi = 300)


# 2. Sex stratum -----
sex_stratum <- read_csv("output/st04_04_stratum_sex.csv") %>%
      arrange(model) %>% relocate(stratum) %>%
      dplyr::select(stratum, model, estimate, lci, hci) %>%
      mutate(`OR (95% CI)` = sprintf("%.2f (%.2f - %.2f)",estimate, lci, hci)) %>%
      mutate(`RR (95% CI)` = `OR (95% CI)` ) %>%
...
ggsave(sex_plots, file = "output/st04_05_stratum_sex_visits.svg",
       device = "svg",
       width=12, height=5, units = "in", dpi = 300)


# 3. Age categories: ------
age_stratum <- read_csv("output/st04_04_stratum_age.csv")%>%
      arrange(model) %>% relocate(stratum) %>%
      dplyr::select(stratum, model, estimate, lci, hci) %>%
      mutate(`OR (95% CI)` = sprintf("%.2f (%.2f - %.2f)",estimate, lci, hci)) %>%
      mutate(`RR (95% CI)` = `OR (95% CI)` ) %>%
...
ggsave(age_plots, file = "output/st04_05_stratum_age_visits.svg",
       device = "svg",
