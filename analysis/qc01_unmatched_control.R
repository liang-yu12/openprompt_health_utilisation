...
lc_exp_raw <- lc_exp_raw %>% mutate(wrong_date = ifelse((long_covid_dx_date <= end_death |
                                                            long_covid_dx_date <= end_deregist |
                                                            long_covid_dx_date <= end_lc_cure), 1, 0))
...
lc_com_raw <- lc_com_raw %>% mutate(wrong_date = ifelse((long_covid_dx_date <= end_death |
                                  long_covid_dx_date <= end_deregist |
                                  long_covid_dx_date <= end_lc_cure), 1, 0))
...
dependent = "exposure"
explanatory = c("sex", "age", "wrong_date")
