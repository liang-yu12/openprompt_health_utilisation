...
matched_apc_cost_ts$exposure <- factor(matched_apc_cost_ts$exposure, levels = c("Comparator", "Long covid exposure"))

matched_apc_cost_ts$exposure %>% levels
...
for_covariates <- matched_apc_cost_ts %>% distinct(patient_id, exposure, .keep_all = T) %>% 
      dplyr::select("patient_id",     
                    ...
                    "previous_covid_hosp",     
                    "cov_covid_vax_n_cat",     
                    "number_comorbidities_cat", all_of(all_admission))
...
matched_cost_12m <- matched_cost_12m %>% mutate(apc_cost2 = case_when(
      admin_counts > 0 & apc_cost >0 ~ apc_cost,
      admin_counts > 0 & apc_cost ==0 ~ unit_apc_costs$unit_cost*admin_counts,
      admin_counts == 0 & apc_cost ==0 ~ 0,
      admin_counts == 0 & apc_cost > 0 ~ 0))
