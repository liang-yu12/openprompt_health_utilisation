...
dataset_exp_lc_unmatched <- fread(here::here("output","dataset_exp_lc_unmatched.csv.gz"))
...
data.frame(
      ...
      fwrite(here("output", "exp_stp_names_numbers.csv.gz"))
...
lapply(names(exp_split_list), function(x) {
      fwrite(as.list(exp_split_list[[x]]), file = here("output", paste0("exp_stp_", x, ".csv.gz")),
             row.names = FALSE)
})
...
# Step 1: Read in the csv file using fread
dataset_comparator_unmatched <- fread(here::here("output", "dataset_comparator_unmatched.csv.gz"))
...
data.frame(
      ...
      fwrite(here("output", "com_stp_names_numbers.csv.gz"))
...
lapply(names(com_split_list), function(x) {
      fwrite(as.list(com_split_list[[x]]), file = here("output", paste0("com_stp_", x, ".csv.gz")),
                row.names = FALSE)
})
