...
matched_data_opa_12m <- matched_data_opa_12m %>% 
      mutate(non_zero = ifelse(visits>0, 1, 0))

opa_summary_12m <- matched_data_opa_12m %>% group_by(region,exposure) %>% 
      summarise(
            opa_median = median(visits),
                opa_mean = mean(visits, na.rm =T),
                opa_max = max(visits),
...
opa_non_zero_12m <-  matched_data_opa_12m %>% filter(non_zero>0) %>% 
      ...
      summarise(
            ...
            high_use= sum(visits>=4*12, na.rm = T),
            fu_time = mean(follow_up, na.rm =T)) %>% 
      as.data.frame() %>% mutate(data = "12 month - non_zero")
...
hx_data <- hx_data %>% mutate(non_zero = ifelse(total_hx_opa>0, 1, 0))
...
explore_hx_opa_non_zero <- hx_data %>% filter(non_zero>0) %>% 
      group_by(region,exposure) %>% 
      summarise(opa_median = median(total_hx_opa),
                opa_mean = mean(total_hx_opa, na.rm =T),
                opa_max = max(total_hx_opa),
...
(hx_data %>% filter(non_zero>0) %>% 
      ggplot(aes(x=total_hx_opa, color = exposure, fill = exposure)) + 
      geom_histogram(position = "dodge") +
      ggtitle("Historical OPA visits")), ncol = 2, nrow = 1)
