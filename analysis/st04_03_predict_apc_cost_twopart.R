source("analysis/dm03_05_now_apc_costs_pivot.R")
...
bind_rows(crude_binomial_apc, adj_binomial_apc) %>% 
      write_csv(here("output", "st03_05_now_apc_cost_binomial_output.csv"))
# Gamma glm part 
bind_rows(crude_gamma_glm_apc, adj_gamma_glm_apc) %>% 
      write_csv(here("output", "st03_05_now_apc_cost_twopm_output.csv"))


# Save the detailed outputs to a text file:
sink(here("output", "st03_05_now_apc_cost_reg_summary.txt"))
...
total_apc_costs %>% write_csv(here("output","st03_05_predict_apc_cost_tpm.csv"))
...
bind_rows(bi_model_count_fn(crude_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
      ...
      write_csv("output/st03_05_apc_binomial_model_counts.csv")
...
bind_rows(gamma_model_count_fn(crude_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
          ...
          gamma_model_count_fn(adj_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_05_apc_gamma_model_counts.csv")
