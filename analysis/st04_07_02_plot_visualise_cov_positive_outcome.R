...
# Binomial models: ---------------
bi_total_visits <- read_csv("output/st04_07_sens_covid_pos_total_binomial.csv") %>% 
...
# Hurdle models: -------------
# Read in results: 
total_hurdle <- read_csv("output/st04_07_sens_covid_pos_total_hurdle.csv") %>% 
      filter(time == "12 months" & Adjustment == "Adjusted") %>% 
      mutate(Group = "Long COVID group") %>% 
      dplyr::select(Group, estimate, lci, hci) %>% 
      add_row(
...
total_predicted_counts <- read_csv("output/st04_07_sens_covid_pos_total_predicted_counts.csv") %>% 
...
ggsave(visits_all_plots, file = "output/st04_07_plot_sens_covid_plus_total_healthcare_visits.svg",
       device = "svg",
