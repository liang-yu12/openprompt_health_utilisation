...
# Binomial models: ---------------
bi_total_visits <- read_csv("output/st02_01_total_binomial_sens_gp_exclude1.csv") %>% 
...
# Hurdle models: -------------
# Read in results: 
total_hurdle <- read_csv("output/st02_01_total_hurdle_sens_gp_exclude1.csv") %>% 
      filter(time == "12 months" & Adjustment == "Adjusted") %>% 
      mutate(Group = "Long COVID group") %>% 
      dplyr::select(Group, estimate, lci, hci) %>% 
      add_row(
...
total_predicted_counts <- read_csv("output/st02_01_total_predicted_counts_sens_gp_exclude1.csv") %>% 
...
ggsave(visits_all_plots, file = "output/st02_06_total_healthcare_visits_sens_gp_exclude1.svg",
       device = "svg",
