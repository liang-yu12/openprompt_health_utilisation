...
# Only run the model among people had been to gp 1 year before:
matched_data_12m <- matched_data_12m %>% filter(reg_and_visit_gp_1y == "TRUE")
...
sink(here("output", "st04_06_sens_gp1y_total_reg_summary.txt"))
...
st03_01_total_binomial %>% write_csv(here("output", "st04_06_sens_gp1y_total_binomial.csv"))

st03_01_total_hurdle <- bind_rows(crude_hurdle_outputs, adj_hurdle_outputs)
st03_01_total_hurdle %>% write_csv(here("output", "st04_06_sens_gp1y_total_hurdle.csv"))
...
bind_rows(crude_summarised_results, adj_summarised_results) %>% 
  write_csv(here("output", "st04_06_sens_gp1y_total_predicted_counts.csv"))
...
bind_rows(
      ...
      write_csv("output/st04_06_sens_gp1y_total_binomial_model_counts.csv")
...
bind_rows(
      ...
      write_csv("output/st04_06_sens_gp1y_total_hurdle_model_counts.csv")
