# Goal: Data management for pivoting parimarycare_visit
# run the analyses using different outcomes: 
# But need to reshape the data by different healthcare visits # parimarycare_visit

# Load previous data management
source("analysis/dm01_02_now_monthly_follow_up.R")

# parimarycare vivists:
parimarycare_visit_cols <- c()
for (i in 1:12){
      parimarycare_visit_cols <- c(parimarycare_visit_cols, paste0("primary_care_visit_m", i))
}
...
#Pivot parimarycare visits in the exposure dataset: ==================
exp_parimarycare_visit_ts <- lc_exp_matched %>% 
      pivot_longer(
            cols = all_of(parimarycare_visit_cols),
            names_to = c("month"),
            values_to = "monthly_parimarycare_visits"
      )
exp_parimarycare_visit_ts$month <- str_sub(exp_parimarycare_visit_ts$month, 21) # remove parimarycare_visit_m
exp_parimarycare_visit_ts$month <- as.numeric(exp_parimarycare_visit_ts$month)
...
exp_parimarycare_long <- left_join(exp_parimarycare_visit_ts, exp_fu_ts,
                      by = c("patient_id" = "patient_id", "month" = "month_fu")
)
exp_parimarycare_long %>% names # looks good

rm(exp_parimarycare_visit_ts) # housekeeping

# Pivot the comparator ==========
com_parimarycare_visit_ts <- com_matched %>% 
      pivot_longer(
            cols = all_of(parimarycare_visit_cols),
            names_to = c("month"),
            values_to = "monthly_parimarycare_visits"
      )
com_parimarycare_visit_ts$month <- str_sub(com_parimarycare_visit_ts$month, 21) # remove parimarycare_visit_m
com_parimarycare_visit_ts$month <- as.numeric(com_parimarycare_visit_ts$month)
...
# Combine the data: =============
com_parimarycare_long <- left_join(com_parimarycare_visit_ts, com_fu_ts,
                      by = c("patient_id" = "patient_id", "month" = "month_fu")
)
com_parimarycare_long %>% names
com_parimarycare_long$follow_up_time %>% summary


# Combine two datasets:
matched_data_parimarycare_ts <- bind_rows(exp_parimarycare_long, com_parimarycare_long)

matched_data_parimarycare_ts$exposure <- factor(matched_data_parimarycare_ts$exposure, levels = c("Comparator", "Long covid exposure"))
matched_data_parimarycare_ts$exposure %>% levels
...
# follow 12 months 
matched_data_parimarycare_12m <- matched_data_parimarycare_ts %>% 
      filter(!is.na(follow_up_time)) %>% 
      group_by(patient_id, exposure) %>% 
      summarise(
            visits = sum(monthly_parimarycare_visits),
            follow_up = sum(follow_up_time)) %>% 
      ungroup()


# # Add covariates for adjustment
for_covariates <- matched_data_parimarycare_ts %>% distinct(patient_id, exposure, .keep_all = T) %>% 
...
matched_data_parimarycare_12m <- left_join(matched_data_parimarycare_12m, for_covariates,
                                 by = c("patient_id" = "patient_id", "exposure" = "exposure"))


# correct the level of exposure groups
matched_data_parimarycare_12m$exposure <- relevel(matched_data_parimarycare_12m$exposure, ref = "Comparator")
