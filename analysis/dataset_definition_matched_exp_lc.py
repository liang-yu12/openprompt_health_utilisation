...
with open("output/matched_matches.csv") as csv_file:
    reader = csv.DictReader(csv_file)
    lc_pt_id = [int(row["patient_id"]) for row in reader]

matched_lc_pt = patients.where(patients.patient_id.is_in(lc_pt_id))
...
# define end date:  -----
# lc dx date +12 | death | derigistration | post COVID-19 syndrome resolved
...
lc_cure = clinical_events.where(clinical_events.snomedct_code ==  SNOMEDCTCode("1326351000000108")) \
    .sort_by(clinical_events.date) \
    .first_for_patient()
# # need to choose the earliest as the study end date
