...
crude_ane_complete_12m <- matched_data_ae_12m %>% drop_na(any_of(crude_vars)) %>% 
...
pos_negbinomial <- vglm(visits ~ exposure + offset(log(follow_up)),
                     ...
                     data = subset(crude_ane_complete_12m, visits_binary > 0))
...
) %>% write_csv(here("output", "qc06_ane_vist_model_compare.csv"))
