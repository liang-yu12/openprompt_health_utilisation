...
crude_binomial_opa <- tidy_binomial_fn(crude_bi_12m) %>% mutate(time = "12 months") %>% 
...
crude_gamma_glm_opa <- tidy_gamma_glm_fn(crude_gamma_12m) %>% mutate(time = "12 months") %>%
      mutate(adjustment = "Crude") %>% 
...
adj_binomial_opa <- tidy_binomial_fn(adj_bi_12m) %>% mutate(time = "12 months") %>% 
...
# Organise the output:
adj_gamma_glm_opa <- tidy_gamma_glm_fn(adj_gamma_12m) %>% mutate(time = "12 months") %>% 
      mutate(adjustment = "Adjusted")%>% 
      arrange(desc(term =="exposureLong covid exposure")) 
...
bind_rows(crude_binomial_opa, adj_binomial_opa) %>% 
      write_csv(here("output", "st03_07_v2_inputed_opa_cost_binomial_output.csv"))

# # Gamma GLM part
bind_rows(crude_gamma_glm_opa,adj_gamma_glm_opa) %>% 
      write_csv(here("output", "st03_07_v2_inputed_opa_cost_twopm_output.csv"))


# Save the detailed outputs to a text file:
sink(here("output", "st03_07_v2_inputed_opa_reg_summary.txt"))
...
# Crude costs: ----
crude_opa_costs <- predict_avg_opa_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
# Adjusted costs: ----
# combine outputs
adj_opa_costs <- predict_avg_opa_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
total_opa_costs %>% write_csv(here("output","st03_07_v2_inputed_predict_opa_cost_tpm.csv"))
...
gamma_model_count_fn <- function(data){
      ...
      data %>% filter(cost_binary>0) %>% 
            group_by(exposure) %>% 
            summarise(
                  mean_cost = mean(opa_cost),
                  min_cost = min(opa_cost),
                  max_cost = max(opa_cost),
                  n = n(),
...
bind_rows(
      bi_model_count_fn(crude_opa_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Crude"),
      bi_model_count_fn(adj_opa_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_07_v2_inputed_opa_binomial_model_counts.csv")


bind_rows(
      gamma_model_count_fn(crude_opa_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Crude"),
      gamma_model_count_fn(adj_opa_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_07_v2_inputed_opa_gamma_model_counts.csv")
