source("analysis/dm03_04_now_pivot_drug_costs_pivot.R")
...
crude_vars <- c("drug_cost", "exposure", "follow_up")

crude_drug_cost_complete_12m <- matched_cost_12m %>% drop_na(any_of(crude_vars)) %>% 
      mutate(cost_binary = ifelse(drug_cost>0, 1, 0)) 
...
crude_bi_12m <- crude_bi_fn(crude_drug_cost_complete_12m)
...
crude_gamma_fn <- function(dataset){
      glm(drug_cost ~ exposure + offset(log(follow_up)),
          data = subset(dataset, cost_binary>0),
          family = Gamma(link="log"))
}

# run crude gamma GLM 
crude_gamma_12m <- crude_gamma_fn(crude_drug_cost_complete_12m)
...
adj_drug_cost_complete_12m <- matched_cost_12m[complete.cases(matched_cost_12m),] %>% 
      mutate(cost_binary = ifelse(drug_cost>0, 1, 0))
...
# run model:
adj_bi_12m <- adj_bi_fn(adj_drug_cost_complete_12m)
...
adj_gamma_fn <- function(dataset){
      glm(drug_cost ~ exposure + offset(log(follow_up))+ age + sex  + cov_covid_vax_n_cat + 
                cov_asthma + cov_mental_health + 
                bmi_cat + imd_q5 + ethnicity_6 + region + number_comorbidities_cat,
          data = subset(dataset, cost_binary>0),
          family = Gamma(link="log")) 
...
adj_gamma_12m <- adj_gamma_fn(adj_drug_cost_complete_12m)
...
bind_rows(crude_binomial_gp,
          adj_binomial_gp) %>% 
      write_csv(here("output", "st03_04_now_drug_cost_binomial_output.csv"))


bind_rows(crude_gamma_glm_gp, adj_gamma_glm_gp) %>% 
      write_csv(here("output", "st03_04_now_drug_cost_twopm_output.csv"))


# Save the detailed outputs to a text file:
sink(here("output", "st03_04_now_drug_reg_summary.txt"))
...
# write function: 
predict_avg_drug_cost_fn <- function(dataset, fu_time, first_reg, sec_reg){
      ...
      input <- input %>% mutate(
            c_drug_cost = nonzero_chance*twopm_cost,
            c_drug_cost_lci =nonzero_chance*twopm_cost_lci,
            c_drug_cost_hci =nonzero_chance*twopm_cost_hci,)
      
      # Summarise the output:
      results <- input %>% group_by(exposure) %>% 
            summarise(cost=mean(c_drug_cost, na.rm =T),
                      lci=mean(c_drug_cost_lci, na.rm =T),
                      uci=mean(c_drug_cost_hci, na.rm =T)
            )
      return(results)
}
...
# Crude costs: ----
crude_drug_costs <- predict_avg_drug_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
# Adjusted costs: ----
# combine outputs
adj_drug_costs <- predict_avg_drug_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
total_drug_costs <- bind_rows(crude_drug_costs, adj_drug_costs) 

total_drug_costs %>% write_csv(here("output","st03_04_predict_drug_cost_tpm.csv"))
...
gamma_model_count_fn <- function(data){
      ...
      data %>% filter(cost_binary>0) %>% 
            group_by(exposure) %>% 
            summarise(
                  mean_cost = mean(drug_cost),
                  min_cost = min(drug_cost),
                  max_cost = max(drug_cost),
                  n = n(),
...
bind_rows(bi_model_count_fn(crude_drug_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Crude"),
            bi_model_count_fn(adj_drug_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_04_drug_binomial_model_counts.csv")


bind_rows(gamma_model_count_fn(crude_drug_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Crude"),
          gamma_model_count_fn(adj_drug_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_04_drug_gamma_model_counts.csv")
