source("analysis/dm03_05_v2_now_input_apc_costs_pivot.R")


matched_cost_12m %>% 
      summarise(
            apc_mean = mean(apc_cost, na.rm = T),
            apc_n = sum(apc_cost>0),
            apc_inputed_mean = mean(apc_cost2,na.rm = T),
            apc_inputed_n = sum(apc_cost2>0)
      ) %>% write_csv(here("output", "st03_05_v2_now_summarising_input_apc_costs.csv"))
...
bind_rows(crude_binomial_apc, adj_binomial_apc) %>% 
      write_csv(here("output", "st03_05_v2_now_input_apc_cost_binomial_output.csv"))
# Gamma glm part 
bind_rows(crude_gamma_glm_apc, adj_gamma_glm_apc) %>% 
      write_csv(here("output", "st03_05_v2_now_input_apc_cost_twopm_output.csv"))


# Save the detailed outputs to a text file:
sink(here("output", "st03_05_v2_now_apc_cost_reg_summary.txt"))
...
total_apc_costs %>% write_csv(here("output","st03_05_v2_now_predict_apc_cost_tpm.csv"))
...
bind_rows(bi_model_count_fn(crude_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
      ...
      write_csv("output/st03_05_v2_now_apc_binomial_model_counts.csv")
...
bind_rows(gamma_model_count_fn(crude_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
          ...
          gamma_model_count_fn(adj_apc_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_05_v2_now_apc_gamma_model_counts.csv")
