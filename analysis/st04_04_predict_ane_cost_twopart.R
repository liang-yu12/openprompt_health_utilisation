source("analysis/dm03_06_now_ane_costs_pivot.R")
...
bind_rows(crude_binomial_ane, adj_binomial_ane) %>% 
      write_csv(here("output", "st03_06_ane_cost_binomial_output.csv"))

# # Gamma GLM part
bind_rows(crude_gamma_glm_ane, adj_gamma_glm_ane) %>% 
      write_csv(here("output", "st03_06_ane_cost_twopm_output.csv"))

# Save the detailed outputs to a text file:
sink(here("output", "st03_06_ane_reg_summary.txt"))
...
total_ane_costs %>% write_csv(here("output","st03_06_predict_ane_cost_tpm.csv"))
...
bind_rows(
      ...
      write_csv("output/st03_06_ane_binomial_model_counts.csv")
...
bind_rows(
      ...
      gamma_model_count_fn(adj_ane_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_06_ane_gamma_model_counts.csv")
