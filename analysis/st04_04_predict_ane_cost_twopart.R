...
# Goal: analysing long COVID exposure and the A&E cost outcomes
# Model: two-part model 

# Data management: colllapsing data by different follow-up time. -------
# 3 months 
matched_cost_3m <- matched_ane_cost_ts %>% 
...
# 6 months
matched_cost_6m <- matched_ane_cost_ts %>% 
...
crude_vars <- c("ane_cost", "exposure", "follow_up")

crude_ane_cost_complete_3m <- matched_cost_3m %>% drop_na(any_of(crude_vars)) %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0))
crude_ane_cost_complete_6m <- matched_cost_6m %>% drop_na(any_of(crude_vars)) %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0))
crude_ane_cost_complete_12m <- matched_cost_12m %>% drop_na(any_of(crude_vars)) %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0)) 
...
crude_bi_3m <- crude_bi_fn(crude_ane_cost_complete_3m)
crude_bi_6m <- crude_bi_fn(crude_ane_cost_complete_6m)      
crude_bi_12m <- crude_bi_fn(crude_ane_cost_complete_12m)
...
crude_gamma_fn <- function(dataset){
      glm(ane_cost ~ exposure + offset(log(follow_up)),
          data = subset(dataset, cost_binary>0),
          family = Gamma(link="log"))
}

# run crude gamma GLM 
crude_gamma_3m <- crude_gamma_fn(crude_ane_cost_complete_3m)
crude_gamma_6m <- crude_gamma_fn(crude_ane_cost_complete_6m)
crude_gamma_12m <- crude_gamma_fn(crude_ane_cost_complete_12m)
...
adj_ane_cost_complete_3m <- matched_cost_3m[complete.cases(matched_cost_3m),] %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0))
adj_ane_cost_complete_6m <- matched_cost_6m[complete.cases(matched_cost_6m),] %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0))
adj_ane_cost_complete_12m <- matched_cost_12m[complete.cases(matched_cost_12m),] %>% 
      mutate(cost_binary = ifelse(ane_cost>0, 1, 0))
...
# run model:
adj_bi_3m <- adj_bi_fn(adj_ane_cost_complete_3m)
adj_bi_6m <- adj_bi_fn(adj_ane_cost_complete_6m)
adj_bi_12m <- adj_bi_fn(adj_ane_cost_complete_12m)

# Organise binomial outputs 
adj_binomial_ane <- bind_rows(
...
adj_gamma_3m <- adj_gamma_fn(adj_ane_cost_complete_3m)
adj_gamma_6m <- adj_gamma_fn(adj_ane_cost_complete_6m)
adj_gamma_12m <- adj_gamma_fn(adj_ane_cost_complete_12m)


# Organise the output:
adj_gamma_glm_ane <- bind_rows(
...
# # Binomial part
bind_rows(crude_binomial_ane, adj_binomial_ane) %>% 
...
# write function: 
predict_avg_ane_cost_fn <- function(dataset, fu_time, first_reg, sec_reg){
      ...
      input <- input %>% mutate(
            c_ane_cost = nonzero_chance*twopm_cost,
            c_ane_cost_lci =nonzero_chance*twopm_cost_lci,
            c_ane_cost_hci =nonzero_chance*twopm_cost_hci,)
      
      # Summarise the output:
      results <- input %>% group_by(exposure) %>% 
            summarise(cost=mean(c_ane_cost, na.rm =T),
                      lci=mean(c_ane_cost_lci, na.rm =T),
                      uci=mean(c_ane_cost_hci, na.rm =T)
            )
      return(results)
}
...
# Crude costs: ----
crude_ane_costs <- bind_rows(
      predict_avg_ane_cost_fn(dataset = matched_cost_3m, 
                             fu_time = 30*3,
                             first_reg = crude_bi_3m, 
                             sec_reg = crude_gamma_3m) %>% mutate(time="3 months"),
      predict_avg_ane_cost_fn(dataset = matched_cost_6m, 
                             fu_time = 30*6,
                             first_reg = crude_bi_6m, 
                             sec_reg = crude_gamma_6m) %>% mutate(time="6 months"),
      predict_avg_ane_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
