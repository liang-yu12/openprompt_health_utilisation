...
# Goal: analysing long COVID exposure and the (inputed) cost outcomes
...
sink(here("output", "st03_03_v2_total_inputed_cost_reg_summary.txt"))
...
all_binary %>% write_csv(here("output", "st03_03_v2_inputed_total_cost_binary.csv"))
all_gamma%>% write_csv(here("output", "st03_03_v2_inputed_total_cost_gammaglm.csv"))
...
predict_avg_cost_fn(dataset = adj_cost_complete_12m,
      ...
      write_csv(here("output", "st03_03_v2_inputed_total_cost_predicted_costs.csv"))
...
bind_rows(
      ...
      write_csv("output/st03_03_v2_inputed_total_cost_binomial_model_counts.csv")
...
bind_rows(
      ...
      gamma_model_count_fn(adj_cost_complete_12m) %>% 
            mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_03_v2_inputed_total_cost_gamma_model_counts.csv")
