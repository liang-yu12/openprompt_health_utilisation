...
# Data management: rename var for model
matched_cost_12m %>% names

matched_cost_12m <- matched_cost_12m %>% mutate(ane_cost = ane_cost_impute_12m)
...
crude_binomial_ane <- tidy_binomial_fn(crude_bi_12m) %>% mutate(time = "12 months") %>% 
...
crude_gamma_glm_ane <- tidy_gamma_glm_fn(crude_gamma_12m) %>% mutate(time = "12 months") %>%
      mutate(adjustment = "Crude") %>% 
...
adj_binomial_ane <- tidy_binomial_fn(adj_bi_12m) %>% mutate(time = "12 months") %>% 
...
# Organise the output:
adj_gamma_glm_ane <- tidy_gamma_glm_fn(adj_gamma_12m) %>% mutate(time = "12 months") %>% 
      mutate(adjustment = "Adjusted")%>% 
      arrange(desc(term =="exposureLong covid exposure")) 
...
bind_rows(crude_binomial_ane, adj_binomial_ane) %>% 
      write_csv(here("output", "st03_06_v2_inputed_ane_cost_binomial_output.csv"))

# # Gamma GLM part
bind_rows(crude_gamma_glm_ane, adj_gamma_glm_ane) %>% 
      write_csv(here("output", "st03_06_v2_inputed_ane_cost_twopm_output.csv"))

# Save the detailed outputs to a text file:
sink(here("output", "st03_06_v2_inputed_ane_reg_summary.txt"))
...
# Crude costs: ----
crude_ane_costs <- predict_avg_ane_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
# Adjusted costs: ----
# combine outputs
adj_ane_costs <- predict_avg_ane_cost_fn(dataset = matched_cost_12m, 
                             fu_time = 30*12,
...
total_ane_costs %>% write_csv(here("output","st03_06_v2_inputed_predict_ane_cost_tpm.csv"))
...
bind_rows(
      ...
      write_csv("output/st03_06_v2_inputed_ane_binomial_model_counts.csv")
...
bind_rows(
      ...
      gamma_model_count_fn(adj_ane_cost_complete_12m) %>% mutate(time = "12m") %>% 
            mutate(model = "Adjusted")) %>% 
      write_csv("output/st03_06_v2_inputed_ane_gamma_model_counts.csv")
